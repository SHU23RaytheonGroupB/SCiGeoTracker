...
function _timeline(Timeline, myData, oneYearAgo, oneYearFromNow) {
  return Timeline(myData, {
    from: oneYearAgo,
    until: oneYearFromNow,
  });
}

function _4(timeline) {
...
function _oneYearFromNow() {
  return END_DATE;
}

function _Timeline(d3, width) {
  return function Timeline(data, options) {
    const axis = {};
    const nodes = {};
    let _data = data;
...
async function _myData() {
  ...
  return Array.from({ length: allProducts.length }, (x, i) => ({
    id: allProducts[i].identifier,
    start: new Date(new Date(allProducts[i].objectstartdate)),
  }));
}
...
export default function define(runtime, observer) {
  ...
  main
    .variable(observer("timeline"))
    .define("timeline", ["Timeline", "myData", "oneYearAgo", "oneYearFromNow"], _timeline);
