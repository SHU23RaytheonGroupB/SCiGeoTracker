...
import { updateUkArea, updateArea } from "./area-calculations.js";
...
function drawPoly() {
  ...
  map.on("draw.create", () => updateArea(allProducts, draw.getAll()));
  map.on("draw.delete", () => updateArea(allProducts, draw.getAll()));
  map.on("draw.update", () => updateArea(allProducts, draw.getAll()));
  map.on("draw.selectionchange", () => updateArea(allProducts, draw.getAll()));
}
...
const borderSelectionMode = () => {
  ...
  updateUkArea(allProducts, UKlandBorder);
};
