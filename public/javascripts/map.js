...
function updateArea(e) {
  ...
  if (data.features.length > 0 && data.features[0].geometry.coordinates.length > 0) {
    ...
    const uncoveredArea = Math.round((area - coveredArea) * 100) / 100;
    ...
    document.querySelector("#area-selection-total-area").textContent = `${roundedArea.toLocaleString()}km²`;
    document.querySelector("#area-selection-covered-area").textContent = `${coveredArea.toLocaleString()}km²`;
    document.querySelector("#area-selection-uncovered-area").textContent = `${uncoveredArea.toLocaleString()}km²`;
